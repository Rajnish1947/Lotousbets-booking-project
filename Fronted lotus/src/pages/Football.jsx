...
const Football = () => {
    ...
    async function fetchMatches() {
      try {
        ...
        if (json.message === "In Play Data fetched Successfully") {
          setInPlayMatches(
            json.data.sportsEventModelInPlay.filter((m) => m.eid === "1")
          );
          setUpcomingMatches(
            json.data.sportsEventModelToday.filter((m) => m.eid === "1")
          );
        }
      } catch (error) {
  ...
  const handleRowClick = (match) => {
    const id = match.gameId || match.SrNo || "unknown";

    // Assuming marketId is a direct property of match, fallback to empty string
    ...
    matches.map((match) => (
      ...
      >
        ...
        <div className="col-span-2 border-r border-gray-200 font-semibold flex items-center p-2">
          {match.eventName || "Football Match"}
        </div>
        ...
        {match.Status !== 1 ? (
          ...
          <div className="col-span-2 md:col-span-4 grid grid-cols-3 md:grid-cols-6 text-center font-bold m-1 gap-1">
            ...
            <div className="hidden md:block p-1 bg-green-300 border border-gray-300">
              {match.lay1 || "-"}
            </div>
            ...
            <div className="hidden md:block p-1 bg-red-100 border border-gray-300">
              {match.lay12 || "-"}
              <p className="text-[8px] md:text-[10px] font-bold text-gray-700">
                {match.lay12Size || " "}
              </p>
            </div>
          </div>
        ...
        <div className="shadow-lg bg-slate-50 rounded-sm w-full lg:max-w-[590px] lg:ml-1 mr-0 mb-5 overflow-hidden">
          ...
          <div className="text-black font-semibold px-2 py-2 flex justify-between items-center border-b border-gray-300 text-lg">
            <div className="flex items-center w-1/2 gap-2">
              <IoIosFootball className="text-green-600" />
              <span>Football</span>
            </div>
